...
![comparison between right and wrong strike orientation](img/kendo-waza/hirauchi-vs-correct.png)
...
graph TD
    %% Global Styling
    classDef foundation fill:#efebe9,stroke:#5d4037,stroke-width:2px,color:#3e2723;
    classDef locomotion fill:#e1f5fe,stroke:#03a9f4,stroke-width:2px,color:#01579b;
    classDef execution fill:#ffebee,stroke:#ef5350,stroke-width:2px,color:#b71c1c;
    classDef warning fill:#fff3e0,stroke:#ffb74d,stroke-width:2px,color:#e65100;

    %% 1. FOUNDATION LEVEL
    Suri["すり足 - Suri-ashi<br>(The Sliding Foundation)"]
    class Suri foundation;

    %% 2. LOCOMOTION LEVEL
    subgraph Locomotion["Ashi-sabaki / 足さばき (Foot work)"]
        direction LR
        Okuri["送り足<br>Okuri-ashi<br>(Default Sliding)"]
        Ayumi["歩み足<br>Ayumi-ashi<br>(Cross Walking)"]
        Hiraki["開き足<br>Hiraki-ashi<br>(Diagonal Evasion)"]
        Tsugi["継ぎ足<br>Tsugi-ashi<br>(Left Foot Cheat)"]
    end
    class Locomotion locomotion;

    %% 3. TERMINATION & EXECUTION LEVEL (Compressed Canvas Fix)
    ...
    %% Apply classes & styles safely
    class Fumikomi execution;
    style ExecutionBox fill:#424242,stroke:#757575,stroke-width:1px,color:#424242;
    style DatotsuLabel fill:none,stroke:none,color:#ffffff;

    %% 4. FOOTWORK SICKNESS
    Itsuku["居付き / Itsuku<br>(The Frozen Foot Sickness)"]
    class Itsuku warning;

    %% Structural Flow Connections
    Suri --> Locomotion
    Okuri --> Fumikomi
    Ayumi --> Fumikomi
    Suri -.-> Itsuku
...
![shikake-ouji-waza](img/kendo-waza/shikake-waza_ojiwaza.jpg)
